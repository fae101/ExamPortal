      ...
      if (x == null || x.IsPublished == false)
            ...
            var examDto = new ExamDto()
            {
                ...
                Questions = x.Questions.Select(q => new QuestionsDTO
                {
                    ...
                    ExamId = q.ExamId,
                    Text = q.Text,
                    Options = q.Options.Split(',').ToList(), // Fix: Convert the string to a List<string> by splitting it
                }).ToList()
            };
            return Ok(examDto);
        }
        ...
        public IActionResult AddExam(CreateExamDTO dto)
        {
            var exam = new Exam
            {
                ...
                {
                    Text = q.Text,
                    Options = string.Join(",", q.Options), // Fix: Convert the List<string> to a string by joining it
